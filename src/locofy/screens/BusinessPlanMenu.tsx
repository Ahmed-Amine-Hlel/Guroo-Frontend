...
import { GurooBusinessPlanService } from "../../core/src/adapters/realDependencies/GurooBusinessPlanService.ts";
import { BusinessPlanMapper } from "../../core/src/adapters/realDependencies/mappers/BusinessPlanMapper.ts";
import { CreateBusinessPlanRevisionUseCase } from "../../core/src/usecases/CreateBusinessPlanRevisionUseCase.ts";
...
const BusinessPlanMenu = () => {
  ...
  const handleEditClick = async (
    ...
    if (!isDone && !isPaid) {
      navigate(`/edit-business-plan/${businessPlanUid}`);
    } else if (isDone && !isPaid) {
      navigate(`/payment/${businessPlanUid}`);
    } else if (isDone && isPaid) {
      try {
        const gurooBusinessPlanService = new GurooBusinessPlanService(
          new BusinessPlanMapper()
        );
        const createRevisionUseCase = new CreateBusinessPlanRevisionUseCase(
          gurooBusinessPlanService
        );

        const revision = await createRevisionUseCase.execute(businessPlanUid);
        dispatch(getBusinessPlanAsync());
        navigate(`/edit-business-plan/${revision.uid}`);
      } catch (error) {
        console.error("Failed to create business plan revision:", error);
  ...
  return (
    <div className="flex flex-col md:flex-row gap-[16px] min-h-[calc(100%_-_65px)] xs:max-h-[calc(100%_-_65px)] md:max-h-[calc(100%_-_65px)] bg-[#f4edfb] px-[20px] lg:px-[100px] py-[40px] font-plus-jakarta-sans ">
      ...
      <div className="flex flex-col w-full max-h-screen">
        ...
        <div className="flex items-center justify-end gap-[10px] h-[75px] rounded-[16px] px-[18px] py-[22px] mb-[22px]">
          ...
          {businessPlan?.length === 0 ? (
            ...
            >
              {sortedBusinessPlans?.map((plan) => (
                <div
                  key={plan.uid}
                  className={`flex flex-col justify-between gap-[14px] rounded-[16px] w-full bg-white p-[12px] ${
                    displayType === "grid" ? "h-[165px]" : ""
                  }`}
                >
                  <div
                    className={
                  ...
                  >
                    <div>
                      <div className="bg-[#f4edfb] mr-2 flex items-center justify-center rounded-full text-[#874CC8] font-bold text-[16px] w-[48px] h-[48px]">
                        {plan.title.slice(0, 2).toUpperCase()}
                      </div>
                    </div>
                    <div
                      className={`flex items-center justify-between w-full text-[#5C3C7C] ${
                        displayType === "grid" ? "mt-auto" : ""
                      }`}
                    >
