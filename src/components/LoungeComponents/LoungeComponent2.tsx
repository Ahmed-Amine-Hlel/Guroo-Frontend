...
const LoungeComponent2 = ({ handleBack }: { handleBack: () => void }) => {
  ...
  useEffect(() => {
    boxContents.forEach((content) => {
      ...
      localStorage.setItem(`lounge-${content}`, initialValue);
    });
        ...
        <div className="w-full sm:px-[35spx] mb-[10px]">
          <div className="flex items-center justify-between">
            <label className="block px-[16px] mb-[16px] text-[14px] text-foundation-purple-dark-active">
              Que proposez vous dans votre lounge ?
            </label>

            <Tooltip
              placement="topRight"
              title={
                "Votre lounge est un centre de revenu à lui tout seul ? Cliquez dessus."
              }
          ...
          <div className="mb-6">
            <QuestionAiBox
              message={
                "Guroo vous recommande de sélectionner au minimum le déjeuner pour votre activité car tous les lounges dans votre zone géographique vous le proposent."
              }
            />
          </div>
        </div>
...
export default LoungeComponent2;
